...
uniform float scanlines_speed : hint_range(0.0, 2.0, 0.01) = float(1.0);
...
void DrawScanline(inout vec3 color, vec2 uv, float time) {
	...
	float grille = (grille_opacity - 0.05) + 0.05 * clamp(1.5 * sin(3.1415926535 * uv.x * screen_size.x), 0.0, 1.0);
